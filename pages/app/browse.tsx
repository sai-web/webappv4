...
const Pages: React.FC<{ PageArr: Array<boolean>, setPageMode: React.Dispatch<React.SetStateAction<Array<boolean>>> }> = ({ PageArr, setPageMode }: { PageArr: Array<boolean>, setPageMode: React.Dispatch<React.SetStateAction<Array<boolean>>> }) => {
    return (
        <>
            <PageModes name="Categories" color={PageArr[0] ? "grey" : undefined} setPageModes={setPageMode} pos={0} logo={<span className="material-icons" style={{ fontSize: "12px" }}>bookmark</span>} />
            <PageModes name="Creators" color={PageArr[1] ? "grey" : undefined} setPageModes={setPageMode} pos={1} logo={<i className="fa fa-user" style={{ fontSize: "10px" }}></i>} />
        </>
    )
}
...
function Browse() {
    ...
    return (
        <Template PageMode={<Pages PageArr={Modes} setPageMode={setPageMode} />} width="180px" >
            <div style={{
                width: "100%",
                height: "200px",
                position: "absolute",
